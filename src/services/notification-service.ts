...
export const registerFCMToken = async (): Promise<boolean> => {
  try {
    ...
    // Verify auth state before attempting registration
    const { useAuthStore } = require('@/src/store/auth-task-store');
    const authState = useAuthStore.getState();
    console.log('🔐 Auth state during FCM registration:', {
      hasToken: !!authState.token,
      isAuthenticated: authState.isAuthenticated,
      userId: authState.user?.id || authState.user?._id,
      userEmail: authState.user?.email,
      tokenPreview: authState.token?.substring(0, 30) + '...',
    });

    if (!authState.token) {
      console.warn('⚠️ No auth token available - FCM registration skipped');
      return false;
    }
    ...
    console.log('📤 Sending FCM token to backend:', {
      fcmTokenPreview: token.substring(0, 20) + '...',
      device,
      deviceId,
      authTokenPreview: authState.token.substring(0, 20) + '...',
    });
...
const setupFirebaseHandlers = (queryClient?: any) => {
  ...
  // ==================== GLOBAL NOTIFICATION HANDLER (iOS + Android) ====================
  // expo-notifications overrides the native UNUserNotificationCenter delegate on iOS.
  // Without this handler, expo-notifications silently swallows notifications.
  // Must be set ONCE before any notification arrives.
  try {
  ...
  messaging().onMessage(async (remoteMessage: any) => {
    ...
    // Create a local notification to display in device notification center.
    // - Android: ALWAYS needed (foreground FCM messages don't auto-display)
    // - iOS with notification payload: Native willPresent handles display, skip to avoid duplicate
    // - iOS data-only (no notification payload): Need local notification since nothing shows natively
    const hasNotificationPayload = !!remoteMessage.notification?.title || !!remoteMessage.notification?.body;
    const shouldCreateLocalNotification = Platform.OS === 'android' || (Platform.OS === 'ios' && !hasNotificationPayload);
    
    if (shouldCreateLocalNotification) {
      try {
        const Notifications = require('expo-notifications');
        ...
        console.log(`✅ Foreground notification displayed on ${Platform.OS}`);
      } catch (error) {
        console.error('❌ Error displaying foreground notification:', error);
      }
    } else {
      console.log(`ℹ️ [${Platform.OS}] Skipping local notification — native willPresent handles display`);
    }
  });
