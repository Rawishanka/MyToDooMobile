...
const MessageScreen: React.FC = () => {
  ...
  const [localMessagePreviews, setLocalMessagePreviews] = useState<
  ...
  const chatMessages: Message[] = useMemo(() => {
    ...
    try {
      const filteredChats = chatData.data.filter((chatItem: ChatListItem) => {
        ...
        const isAcceptedOffer =
          taskStatus && validStatuses.includes(taskStatus);
      ...
      return filteredChats.map((chatItem: ChatListItem) => {
        ...
        let avatarUrl =
          "https://ui-avatars.com/api/?name=User&background=007AFF&color=fff&size=100";
        ...
        if ((otherParticipant as any)?.avatar) {
          ...
          avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(
            name
          )}&background=007AFF&color=fff&size=100&bold=true&rounded=true`;
        }
        ...
        return {
          ...
          preview: preview,
          date: new Date(
            chatItem.lastMessage?.timestamp || chatItem.chat.createdAt
    ...
    <View style={styles.container}>
      ...
      {isLoadingChats ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <MessageListItem message={item} onPress={handleMessagePress} />
          )}
