...
export default function LoginScreen() {
  ...
  const handleGoogleSignInSuccess = useCallback(async (idToken: string) => {
    try {
      ...
      // Wait for auth store to be fully updated (Google Sign-In needs more time
      // because setAuthData writes to both Zustand + AsyncStorage)
      ...
      // 🔔 Register FCM token after Google Sign-In with retry
      // This ensures notifications work after switching sign-in methods
      if (authState.token && authState.isAuthenticated) {
        let fcmRegistered = false;
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            console.log(`🔔 [Attempt ${attempt}/3] Registering FCM token after Google Sign-In...`);
            fcmRegistered = await registerFCMToken();
            if (fcmRegistered) {
              console.log(`✅ FCM token registered after Google Sign-In (attempt ${attempt})`);
              break;
            } else {
              console.warn(`⚠️ FCM token registration returned false (attempt ${attempt})`);
            }
          } catch (fcmError) {
            console.warn(`⚠️ FCM registration attempt ${attempt} failed:`, fcmError);
          }
          // Wait before retry
          if (attempt < 3) {
        ...
        if (!fcmRegistered) {
          console.error('❌ FCM token registration failed after 3 attempts for Google Sign-In user');
        }
      } else {
        console.error('❌ Auth state not ready for FCM registration after Google Sign-In:', {
          hasToken: !!authState.token,
          isAuthenticated: authState.isAuthenticated,
        });
      }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 🔔 Explicitly re-register FCM token after normal login
      try {
        console.log('🔔 Re-registering FCM token after email login...');
        await registerFCMToken();
        console.log('✅ FCM token re-registered after email login');
      } catch (fcmError) {
  ...
  const handleAppleSignIn = async () => {
    try {
      ...
      // 🔔 Explicitly re-register FCM token after Apple Sign-In
      try {
        console.log('🔔 Re-registering FCM token after Apple Sign-In...');
        await registerFCMToken();
        console.log('✅ FCM token re-registered after Apple Sign-In');
      } catch (fcmError) {
