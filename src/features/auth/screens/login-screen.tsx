...
} from "@/src/shared/utils/pending-action-utils";
  ...
  useEffect(() => {
    ...
    if (response?.type === "success") {
      ...
      if (errorMsg.includes("invalid_request") || errorMsg.includes("400")) {
        ...
        errorDescription.includes("access_denied")
      ) {
        Alert.alert(
          "Access Denied",
      ...
      if (error?.response?.status === 400 || error?.response?.status === 401) {
        ...
        if (
          ...
          Alert.alert(
            "Login Failed",
            backendMessage ||
              "Unable to log in with the provided credentials. Please verify your email and password.",
            [{ text: "OK" }]
          );
        }
        ...
        <View style={styles.header}>
          ...
          <View style={styles.logoContainer}>
            <View style={styles.logoBackground}>
              <MyToDooLogo width={60} height={60} />
            </View>
          </View>
