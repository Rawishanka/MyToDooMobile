...
export default function TaskCard({ task, onPress, status, userRole, onTaskCancelled, onTaskDeleted, onTaskCompleted, myOffer, onOfferDeleted, autoPromptReview = false }: TaskCardProps) {
  ...
  // Check if CURRENT USER has a pending/admin_review cancellation request (waiting for other party or admin)
  const hasOwnPendingCancelRequest = useMemo(() => {
    if (!pendingCancellationRequest) return false;
    if (status === 'cancelled') return false;
    if (!['pending', 'admin_review'].includes(pendingCancellationRequest.status)) return false;
    ...
    return requesterId === currentUserId;
  }, [pendingCancellationRequest, currentUser, status]);
  ...
  // Poster confirms task completion (PATCH /tasks/{taskId}/confirm-completion)
  const handleConfirmCompletion = useCallback(async () => {
    if (confirmTaskCompletionMutation.isPending || isProcessing) {
      console.log('🛡️ Confirm completion already in progress');
      return;
    }
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setIsProcessing(true);
              console.log('✅ Poster confirming task completion:', task._id);
              await confirmTaskCompletionMutation.mutateAsync(task._id);
              console.log('✅ Task completion confirmed by poster');
  ...
  return (
    <View style={[
      ...
      {/* Admin Review Banner - shown to BOTH parties when cancellation is under admin review */}
      {pendingCancellationRequest?.status === 'admin_review' && status !== 'cancelled' && (
        <View style={styles.ownCancelPendingBanner}>
          <MaterialIcons name="hourglass-empty" size={20} color="#fff" />
          <Text style={styles.cancelRequestBannerText}>
            Cancellation request under admin review
          </Text>
        </View>
      )}

      {/* Own Pending Cancellation Banner - when current user sent request and is waiting (pending only) */}
      {hasOwnPendingCancelRequest && pendingCancellationRequest?.status === 'pending' && (
        <View style={styles.ownCancelPendingBanner}>
          <MaterialIcons name="hourglass-empty" size={20} color="#fff" />
          <Text style={styles.cancelRequestBannerText}>
            ...
            {(() => {
              ...
              if (displayUser) {
                ...
                const profileUri = displayUser.avatar || 
                                 ...
                                 `https://ui-avatars.com/api/?name=${formatAvatarName(firstName, lastName)}&background=003399&color=fff&size=80`;
