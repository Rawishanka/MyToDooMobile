/**
 * TaskMapModal
 * Full-screen in-app OpenStreetMap modal for task detail "View on Map" button.
 * Reuses the same MapView (Leaflet/WebView) as the Browse Tasks screen.
 * Focused and zoomed in on the single task location.
 */
import MapView from '@/src/features/tasks/screens/browse/components/MapView';
...
// Same airtasker marker SVG used in Browse Tasks screen
...
const TaskMapModal: React.FC<TaskMapModalProps> = ({ visible, onClose, task }) => {
  const router = useRouter();

  const handleMapAction = (action: string, taskId: string) => {
    if (action === 'viewDetails') {
      // Close the map modal then navigate to task detail
      onClose();
      router.push({
      ...
      const coords = task.location?.coordinates as { type: string; coordinates: [number, number] } | undefined;
      if (coords && 'coordinates' in coords && Array.isArray(coords.coordinates)) {
        ...
        Linking.canOpenURL(nativeUrl!)
          .then((supported) =>
            supported ? Linking.openURL(nativeUrl!) : Linking.openURL(browserUrl)
          )
  ...
  return (
    ...
    >
      <SafeAreaView style={styles.container}>
        ...
        {/* Map — full screen, single task focused */}
        <View style={styles.mapContainer}>
          <MapView
            tasks={[task]}
            iconUrl={MARKER_ICON_URI}
