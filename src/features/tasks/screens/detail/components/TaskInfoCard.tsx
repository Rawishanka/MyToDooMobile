...
export const TaskInfoCard: React.FC<TaskInfoCardProps> = ({
  ...
  return (
    <View style={[styles.taskCard, isDarkMode && { backgroundColor: '#1E293B', borderColor: '#334155' }]}>
            ...
            source={{ 
              uri: (() => {
                ...
                if (task.createdBy?.avatar?.startsWith?.('data:')) {
                  ...
                  return `https://ui-avatars.com/api/?name=${formatAvatarName(firstName, lastName)}&background=003399&color=fff&size=120`;
                }
              })()
            }}
      ...
      {/* View on Map Button */}
      {(() => {
        const coords = parsedLocation?.coordinates as any;
