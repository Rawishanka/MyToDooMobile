...
export default function TaskDetailScreen() {
  ...
  // 2. Task shows current user as assignee, OR
  // 3. Task is cancelled (no one can ask/answer on a cancelled task)
  const isCancelledTask = task.status === 'cancelled' || 
                          task.status === 'cancel_request_by_poster' ||
                          task.status === 'pending_completion' ||
                          task.status === 'completed';

  // Ask Question is only allowed when the task is 'open' or 'assigned'/'todo'
  const isTaskQuestionable = task.status === 'open' || 
