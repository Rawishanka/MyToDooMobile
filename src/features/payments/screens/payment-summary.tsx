...
export default function PaymentSummaryScreen() {
  ...
  const isLoading =
    userRole === "Tasker" ? isTaskerPaymentsLoading : isPosterPaymentsLoading;
  const paymentsError =
    userRole === "Tasker" ? taskerPaymentsError : posterPaymentsError;
    ...
    return paymentsData
      .map((payment: any) => {
          ...
          if (locationType === "string") {
              ...
              try {
                ...
                if (parsedLocation.address) {
                  ...
                  if (parsedLocation.country)
                    parts.push(parsedLocation.country);
                  locationText =
                    parts.length > 0 ? parts.join(", ") : trimmedLocation;
                }
              } catch (error) {
            ...
            if (task.location.address) {
              ...
              if (
                ...
                (addressValue.startsWith("{") || addressValue.startsWith("["))
              ) {
                try {
                  ...
                  if (nestedParsed.address) {
                    ...
                    locationText =
                      parts.length > 0 ? parts.join(", ") : addressValue;
                  }
                } catch (error) {
        ...
        return {
          ...
          currency:
            payment.currency || offer.offer?.currency || userCurrencyInfo.code,
          ...
          taskerName:
            userRole === "Tasker"
              ...
              : `${task.assignedTo?.firstName || "Tasker"} ${
          ...
          posterName:
            userRole === "Poster"
              ...
              : `${task.createdBy?.firstName || "Poster"} ${
                  task.createdBy?.lastName || ""
                }`.trim(),
          acceptedDate:
            payment.releasedAt || payment.updatedAt || payment.createdAt,
  ...
  // Calculate totals
  const totalAmount = paymentSummaryItems.reduce(
    (sum, item) => sum + item.offerAmount,
    0
  );
  const completedPayments = paymentSummaryItems.filter(
    (item) => item.paymentStatus === "completed"
  ).length;
  const pendingPayments = paymentSummaryItems.filter(
    (item) => item.paymentStatus === "pending"
  ).length;
  const failedPayments = paymentSummaryItems.filter(
        ...
        <View style={styles.cardDetailsSection}>
          <View style={styles.detailRow}>
            ...
            <Text style={styles.taskLocation} numberOfLines={1}>
              {typeof item.taskLocation === "string"
                ? item.taskLocation
      ...
      <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        <View style={styles.roleSelectorContainer}>
          <TouchableOpacity
            style={[
              styles.roleButton,
              userRole === "Tasker" && styles.activeRole,
            ]}
          ...
          >
            <Text
              style={[
                styles.roleText,
                userRole === "Tasker" && styles.activeRoleText,
              ]}
            >
              Tasker
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.roleButton,
              userRole === "Poster" && styles.activeRole,
            ]}
          ...
          >
            <Text
              style={[
                styles.roleText,
                userRole === "Poster" && styles.activeRoleText,
              ]}
            >
              Poster
            </Text>
