...
import { addToOfflineQueue, detectOperationType } from '@/src/services/offline/offlineStorage';
...
export function createApi(baseURL: string) {
    ...
    axiosInstance.interceptors.response.use(
        ...
        async (error) => {
            ...
            // ─── OFFLINE SYNC: Queue write operations when network is unavailable ───
            if (isNetworkError(error)) {
                const method = (originalRequest?.method || '').toUpperCase();
                const isWriteOperation = ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method);
                const requestUrl_offline = originalRequest?.url || '';
                const contentType = (originalRequest?.headers?.['Content-Type'] || originalRequest?.headers?.['content-type'] || '').toLowerCase();
                
                // Don't queue these endpoints — they're not replayable or not user actions
                const isAuthEndpoint_offline = requestUrl_offline.includes('/auth/') || 
                                              requestUrl_offline.includes('/users/firebase');
                const isCDNUpload = requestUrl_offline.includes('/cdn/') || 
                                   contentType.includes('multipart/form-data');
                const isNotificationEndpoint = requestUrl_offline.includes('/notifications') || 
                                              requestUrl_offline.includes('/fcm') ||
                                              requestUrl_offline.includes('/device-token');
                const isServiceFee = requestUrl_offline.includes('/service-fee');
                
                const shouldQueue = isWriteOperation && 
                                   !isAuthEndpoint_offline && 
                                   !isCDNUpload && 
                ...
                if (shouldQueue) {
                    try {
                        const { type, description, priority } = detectOperationType(method, requestUrl_offline);
                        
                        // Extract a meaningful description from the request data
                        ...
                        if (originalRequest?.data) {
                            try {
                                ...
                                if (data.title) detailedDescription = `${description}: ${data.title}`;
                                else if (data.message) detailedDescription = `${description}: ${data.message.substring(0, 50)}`;
                        ...
                        console.log(`📦 [Offline] Queued ${method} ${requestUrl_offline} for later sync`);
                        
                        // Return a special "queued" response so the UI can show appropriate feedback
                        return Promise.reject({
                            isNetworkError: true,
                            isOfflineQueued: true,
                            message: "You're offline. Your changes have been saved and will sync when you reconnect.",
                            originalError: error
                        });
                    } catch (queueError) {
                        console.warn('⚠️ [Offline] Failed to queue operation:', queueError);
                    }
                }
                
                // For GET requests or if queueing failed, reject with standard network error
                return Promise.reject({
                    isNetworkError: true,
