...
const OBFUSCATION_PATTERNS = [
  ...
  /\b(my|our)\s*(number|email|phone|contact)/gi,
];

export interface ModerationResult {
  isClean: boolean;
  reason?: string;
  detectedType?: 'phone' | 'email' | 'contact';
...
export function moderateContent(text: string): ModerationResult {
  ...
  for (const pattern of PHONE_PATTERNS) {
    ...
    if (match) {
      return {
        isClean: false,
        reason: 'Phone numbers are not allowed. Please keep conversations within the platform for your safety.',
        detectedType: 'phone',
  ...
  for (const pattern of EMAIL_PATTERNS) {
    ...
    if (match) {
      return {
        isClean: false,
        reason: 'Email addresses are not allowed. Please keep conversations within the platform for your safety.',
        detectedType: 'email',
        matchedPattern: match[0]
      };
    }
  ...
  if (containsContactObfuscation(text)) {
    return {
      isClean: false,
      reason: 'Sharing contact information is not allowed. Please keep conversations within the platform for your safety.',
...
export function getBlockedReason(text: string): string {
  ...
  return result.reason || 'Content contains inappropriate information.';
}
