...
export function useDeleteOffer() {
  ...
  return useMutation({
    mutationFn: ({ taskId, offerId }: { taskId: string; offerId: string }) =>
      TaskAPI.deleteOffer(taskId, offerId),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: TASK_QUERY_KEYS.offers(variables.taskId) });
      queryClient.invalidateQueries({ queryKey: TASK_QUERY_KEYS.allOffers(variables.taskId) });
...
/**
 * ✅ Confirm Task Completion Mutation (Poster only)
 * Used when poster confirms task completion after tasker marks it complete
...
export function useConfirmTaskCompletion() {
  ...
  return useMutation({
    mutationFn: (taskId: string) => TaskAPI.confirmTaskCompletion(taskId),
    onSuccess: (data, taskId) => {
      queryClient.invalidateQueries({ queryKey: TASK_QUERY_KEYS.detail(taskId) });
      queryClient.invalidateQueries({ queryKey: TASK_QUERY_KEYS.completionStatus(taskId) });
      queryClient.invalidateQueries({ queryKey: TASK_QUERY_KEYS.myTasks() });
...
export function useGetCancellationRequest(taskId: string, enabled: boolean = true) {
  return useQuery({
    ...
    // Don't retry on auth errors (401) - retrying won't help if token is expired
    retry: (failureCount, error: any) => {
      if (error?.isAuthError || error?.status === 401 || error?.response?.status === 401) {
        return false; // Never retry auth errors
      }
      return failureCount < 1; // Retry once for other errors
    },
    retryDelay: 1000,
    // Never throw errors to the UI - this is a silent background polling query
    throwOnError: false,
    meta: {
      errorMessage: false,
    },
...
export const TaskHooks = {
  ...
  useDeleteOffer,
  useCompleteTask,
  useConfirmTaskCompletion, // NEW: Poster confirms task completion
