...
/**
 * Upload user avatar via CDN
 * Flow: Upload image file directly to /users/avatar endpoint
 * Backend handles CDN upload and returns the avatar URL
 */
export async function uploadUserAvatar(imageUri: string): Promise<UserProfileResponse> {
  try {
    console.log("📸 Uploading user avatar to /users/avatar...");
    ...
    console.log("📤 Uploading avatar file:", { fileName, mimeType });
    
    // Upload to backend - backend will handle CDN upload
    ...
    console.log("🖼️ New avatar URL:", response.data?.data?.avatar);
...
export interface SocialMediaAccount {
  _id: string;
  platform: string; // e.g. 'instagram', 'facebook', 'whatsapp', 'linkedin', 'tiktok', 'twitter'
  profileUrl: string;
}
 ...
 * Returns all active social media accounts configured by admin.
 */
export async function getSocialMediaAccounts(): Promise<SocialMediaAccount[]> {
  try {
    const response = await api.get('/social-media');
    console.log('\u2705 Social media accounts loaded:', response.data?.data?.length);
    return response.data?.data || [];
  } catch (error: any) {
    console.warn('\u26a0\ufe0f Could not load social media accounts:', error?.message);
    return [];
  }
}
 ...
 * Submit account deletion request. Backend sets status to PENDING_REVIEW.
 * Returns 409 if a pending request already exists.
 */
export async function submitDeleteRequest(reason: string): Promise<DeletionRequestResponse> {
  try {
    const response = await api.post('/user/delete-request', { reason });
    console.log('✅ Delete request submitted:', response.data);
    return response.data;
  } catch (error: any) {
    console.error('❌ Submit delete request failed:', error);
    // 409 means a pending request already exists
    if (error?.response?.status === 409) {
      throw { alreadyPending: true, message: error?.response?.data?.message || 'You already have a pending deletion request.' };
    }
    throw error;
  }
}
 ...
 */
export async function getDeleteRequest(): Promise<DeletionRequestResponse> {
  try {
    const response = await api.get('/user/delete-request');
    console.log('✅ Get delete request status:', response.data);
    return response.data;
  } catch (error: any) {
    console.error('❌ Get delete request failed:', error);
    throw error;
  }
}
