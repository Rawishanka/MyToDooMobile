...
class StripeConnectAPIService {
  ...
  /**
   * POST /stripe/connect/update-account
   * Get a Stripe-hosted URL for updating bank account details
   */
  async updateAccount(returnUrl: string, refreshUrl: string): Promise<{ url: string; accountId: string }> {
    try {
      console.log('📤 Getting Stripe update account link...');
      const headers = await this.getAuthHeaders();
      const response = await fetch(`${this.baseURL}/stripe/connect/update-account`, {
        method: 'POST',
        headers,
      ...
      if (!response.ok) {
        ...
        const error: any = new Error(errorData.message || 'Failed to generate update link');
        error.status = response.status;
        throw error;
      }

      const result = await response.json();
      console.log('✅ Got update account link');
      return result.data;
    } catch (error: any) {
